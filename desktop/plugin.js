...
function applyStoredDisabledProviders() {
  try {
    ...
    disabledProvidersAtom.set(Array.isArray(v) ? v : DISABLED_PROVIDERS_DEFAULT)
  } catch {
    /* noop */
  }
}
...
function providerMeta(pid) {
  return (
    PROVIDER_META[pid] || {
      name: pid,
      mono: String(pid || '?').slice(0, 2).toUpperCase(),
    }
  )
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (result?.blocked || result?.code !== 0) {
        throw new Error(result?.hint || result?.output || 'quota status failed')
      }
...
// ---- per-provider statusbar chip (all mode) --------------------------------
...
function ProviderRow({ id, provider }) {
  ...
  if (reason) {
    return jsxs('div', {
      className: 'flex flex-col gap-0.5 rounded-lg border border-(--ui-stroke-secondary) px-3 py-2.5 opacity-60',
      children: [
        jsxs('div', {
          className: 'flex items-center gap-2 text-sm',
          children: [jsx(ProviderBadge, { pid: id }), jsx('span', { className: 'font-medium text-(--ui-text-primary)', children: displayName })],
        }),
        jsx('div', { className: 'pl-7 text-xs text-(--ui-text-tertiary)', children: t('unavailable', reason) }),
      ],
    })
  }
  ...
  if (windows.length === 0 && details.length === 0) {
    return jsxs('div', {
      className: 'flex items-center gap-2 rounded-lg border border-(--ui-stroke-secondary) px-3 py-2.5 text-sm opacity-60',
      children: [
        jsx(ProviderBadge, { pid: id }),
        jsx('span', { className: 'font-medium text-(--ui-text-primary)', children: displayName }),
  ...
  return jsxs('div', {
    className: 'flex flex-col gap-2 rounded-lg border border-(--ui-stroke-secondary) bg-(--ui-bg-elevated) px-3 py-2.5',
    children: [
      jsxs('div', {
        className: 'flex items-center justify-between gap-2',
        children: [
          jsxs('span', {
            className: 'flex items-center gap-2 text-sm font-semibold text-(--ui-text-primary)',
            children: [
              jsx(ProviderBadge, { pid: id }),
              displayName,
              jsx(StatusDot, { tone: worstWindow(provider) == null ? 'muted' : toneForRemaining(worstWindow(provider)) }),
            ],
          }),
          provider.plan ? jsx('span', { className: 'rounded bg-(--ui-control-hover-background) px-1.5 py-0.5 text-[0.6875rem] font-medium text-(--ui-text-secondary)', children: provider.plan }) : null,
        ],
      }),
      ...windows.map((w, i) => {
        const r = remainingPct(w)
            ...
            children: [
              jsxs('div', {
                ...
                children: [
                  ...
                  jsx('span', { className: 'tabular-nums text-[0.8125rem] font-medium', style: { color: toneColor(tone) }, children: r == null ? '—' : `${r}% left` }),
                ],
              }),
      ...
      ...details.map((d, i) =>
        jsx(
          'div',
          { className: 'text-[0.6875rem] text-(--ui-text-tertiary)', children: d },
          `${id}-d-${i}`
        )
      ),
    ],
...
function DisabledProvidersControl() {
  ...
  const allProviders = data && data.providers ? Object.entries(data.providers) : []
  if (allProviders.length === 0) return null
  return jsxs('div', {
    className: 'flex flex-col gap-2',
    children: [
      jsx('span', { className: 'text-xs text-(--ui-text-secondary)', children: t('disabledProvidersLabel') }),
      jsx('div', {
          ...
          return jsx(
            'button',
            {
              ...
              children: jsxs('span', {
                ...
                children: [
                  jsx('span', { className: enabled ? 'text-(--ui-accent)' : 'text-(--ui-text-quaternary)', children: enabled ? '●' : '○' }),
...
function QuotaPane() {
  ...
  const refresh = useMutation({
    mutationFn: async () => {
      ...
      if (result?.blocked || result?.code !== 0) {
        throw new Error(result?.hint || result?.output || 'quota refresh failed')
      }
    },
  ...
  if (view === 'settings') {
    ...
    const all = Object.entries(data.providers).filter(([pid]) => !disabled.includes(pid))
    const withData = all.filter(([, p]) => isConfigured(p))
    const withoutData = all.filter(([, p]) => !isConfigured(p))
    body = jsxs('div', {
      className: 'flex flex-col gap-2',
      children: [
        ...withData.map(([id, p]) => jsx(ProviderRow, { id, provider: p, key: id })),
        withoutData.length > 0
          ? jsxs('details', {
              className: 'mt-1',
              children: [
                jsx('summary', {
                  className: 'cursor-pointer select-none text-[0.6875rem] text-(--ui-text-quaternary) hover:text-(--ui-text-tertiary)',
                  children: t('noDataSection', withoutData.length),
                }),
                jsx('div', {
                  className: 'mt-1.5 flex flex-col gap-2',
                  children: withoutData.map(([id, p]) => jsx(ProviderRow, { id, provider: p, key: id })),
                }),
              ],
            })
          : null,
